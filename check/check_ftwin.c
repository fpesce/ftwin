...
static void copy_file(const char *src_path, const char *dest_path)
{
	...
	return_value =
	    apr_file_open(&dest_file, dest_path, APR_WRITE | APR_CREATE | APR_TRUNCATE | APR_BINARY, APR_OS_DEFAULT,
			  main_pool);
	ck_assert_int_eq(return_value, APR_SUCCESS);
...
START_TEST(test_ftwin_json_output_validation)
{
	...
	const char *argv[] =
	    { "ftwin", "-J", "check/tests/5K_file", "check/tests/5K_file_copy", "check/tests/1K_file" };
